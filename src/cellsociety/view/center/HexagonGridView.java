...
public class HexagonGridView extends GridView{
  ...
  protected void getMousePosOnGrid(MouseEvent mouseEvent){
    getMousePositionForCircles(mouseEvent); //Good approximation. The perfect values could be computed if we have time.
  }

  private double[][] getHexCoords(int i, int j){
    double[][] coords = {{i, i+RADIUS/2, i+3*RADIUS/2, i+1, i+3*RADIUS/2, i+RADIUS/2, i},
